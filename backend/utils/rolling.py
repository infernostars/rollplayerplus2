...
async def roll(ctx, *, arguments: str = None):
    ...
    try:
        seperate_rolls = arguments.split(" ")
    except:
        seperate_rolls = ['d100']  # default to safety
    for index, e in enumerate(seperate_rolls):
        msg.append({'args': e})
        ...
        perish = e.replace('0', ' ').replace('1', ' ').replace('2', ' ').replace('3', ' ').replace('4', ' ').replace(
            '5', ' ').replace('6', ' ').replace('7', ' ').replace('8', ' ').replace('9', ' ').replace(':', ' ').replace(
            'i', ' ').replace(',', ' ').split()
        fiend = e.replace('+', ' ').replace('-', ' ').replace('*', ' ').replace('/', ' ').split()
        if len(perish) != len(fiend):
            await ctx.response.send_message("Errno 01.1: The modifiers are incomplete.");
            return
        else:
            x = 0
            killurself = []
        ...
        if len("".join(perish).replace('d', '')) != len(fiend) - 1:  # and bonus!=None:
            await ctx.response.send_message("Errno 01.2: The modifiers are incomplete.");
            return
        if dice == None:  # this is understandable, but if you do "v!roll d" i hate you
            ...
            await ctx.response.send_message("Errno 02.1: You did the dice command incorrectly.");
            return
        if dice != None:
            ...
            if dice.replace('i', '') != dice:
                ...
                if identity[1].replace(',', '') != identity[1]:  # this is where it actually turns into brainfuck
                    ...
                    if identity[1] != "":
                        try:
                            purpose = int(identity[1])
                        except:
                            purpose = None
                    else:
                        await ctx.response.send_message(
                            "Errno 03.1: Specify which number you want to be modified when you use ``i``.");
                        return
            if purpose != None:
                if int(number) <= 1:
                    await ctx.response.send_message("Errno 04.1: Only use the ``i`` when needed.");
                    return
                else:
                    if bonus is None:
                        await ctx.response.send_message("Errno 04.2: Only use the ``i`` when needed.");
                        return
                    else:
                        if purpose == 0:
                            await ctx.response.send_message("Errno 05.1: Invalid index for individual modifier.");
                            return
            if thanosnap[1].count(":") != 0:
                ...
                try:
                    ...
                    await ctx.response.send_message("Errno 06.1: You used the ``i`` incorrectly.");return
                if dice.replace('i', '') != dice:
                    ...
                    try:
                        ...
                        await ctx.response.send_message("Errno 06.2: You used the ``i`` incorrectly.");return
            else:
                if thanosnap[1] == "" or thanosnap[0] == None:
                    ...
                    try:
                        high = int(ihatemylife[0])
                    except:
                        high = 100
                    finally:
                        low = 1
        if high <= 1:
            await ctx.response.send_message("Errno 07.1: You can't roll a d0 or d1.");
            return
        if number == None or number == "": number = 1  # failsafe
        if int(number) == 0:
            await ctx.response.send_message("Errno 08.1: You can't roll 0 times.");
            return
        if int(number) > 1:
            a = 0
            constantinople = []
        ...
        if bonus == None:  # the killur and self variables are references to a quackity video
            ...
            killur = bonus.replace('0', ' ').replace('1', ' ').replace('2', ' ').replace('3', ' ').replace('4',
                                                                                                           ' ').replace(
                '5', ' ').replace('6', ' ').replace('7', ' ').replace('8', ' ').replace('9', ' ').split()
            self = bonus.replace('+', ' ').replace('-', ' ').replace('*', ' ').replace('/', ' ').split()
        if bonus is not None and (
                killur == None or self == None or bonus.replace('+', '').replace('-', '').replace('*', '').replace('/',
                                                                                                                   '').replace(
                '0', '').replace('1', '').replace('2', '').replace('3', '').replace('4', '').replace('5', '').replace(
                '6', '').replace('7', '').replace('8', '').replace('9', '') != ""):  # uh oh, retard code alert
            await ctx.response.send_message("Errno 09.1: You did the modifiers incorrectly.");
            return
        if bonus is None:
            if int(number) > 1:
            ...
            if len(killur) != len(self):
                await ctx.response.send_message("Errno 01.3: The modifiers are incomplete.");
                return
            if purpose != None:
                if type(purpose) == int:
                    if int(number) < purpose - 1:
                        await ctx.response.send_message("Errno 05.2: Invalid index for individual modifier.");
                        return
                if type(purpose) == list:
                    for p in purpose:
                        if int(number) < p - 1:
                            await ctx.response.send_message("Errno 05.3: Invalid index for individual modifier.");
                            return
            if int(number) > 1:
                if purpose != None:
                    if type(purpose) == list:
                            ...
                            for n in killur:
                                if killur[x] == "+":
                                    ...
                                    try:
                                        ...
                                        await ctx.response.send_message("Errno 11.1: You attempted to divide by zero.");
                                        return
                                x += 1
                    else:
                        y = purpose - 1
                        ...
                        for n in killur:
                            if killur[x] == "+":
                                ...
                                try:
                                    ...
                                    await ctx.response.send_message("Errno 11.2: You attempted to divide by zero.");
                                    return
                            x += 1
                else:
                    y = 0
                        ...
                        for n in killur:
                            if killur[x] == "+":
                                ...
                                try:
                                    ...
                                    await ctx.response.send_message("Errno 11.3: You attempted to divide by zero.");
                                    return
                            x += 1
                        y += 1
            else:
                ...
                for n in killur:
                    if killur[x] == "+":
                        ...
                        try:
                            ...
                            await ctx.response.send_message("Errno 11.4: You attempted to divide by zero.");
                            return
                    x += 1
                ...
                for n in killur:
                    if killur[x] == "+":
                        ...
                        try:
                            ...
                            await ctx.response.send_message("Errno 11.5: You attempted to divide by zero.");
                            return
                    x += 1
                msg[index]['out'] = f"{english(istanbul)}. [total **{sum(istanbul)}**]"
                msg[index][
                    'raw'] = f"{english(constantinople)}, [total **{sum(constantinople)}**]"  # [modded total **{tot}**] (decided not to keep this)
                msg[index]['level'] = unlerp(low * int(number), high * int(number),
                                             sum(constantinople))  # trust me anything more complex would be programming purgatory
            else:
                x = 0
                tot = i
                for n in killur:
                    if killur[x] == "+":
                        ...
                        try:
                            ...
                            await ctx.response.send_message("Errno 11.6: You attempted to divide by zero.");
                            return
                    x += 1
                msg[index]['out'] = f"**{e}**"
                msg[index]['raw'] = f"**{i}**"
    ...
    embed = discord.Embed(title="Here are your rolls!",
                          color=round(color[0]) * 0x10000 + round(color[1]) * 0x100 + round(
                              color[2]))  # color wizardry don't worry about it
    for i in msg:
        if 'raw' in list(i.keys()):
            embed.add_field(name=f"You rolled [{i['args']}] and got...", value=f"{i['out']}, with raw {i['raw']}.",
                            inline=False)
        else:
